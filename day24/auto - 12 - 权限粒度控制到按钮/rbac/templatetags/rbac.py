...
@register.filter
def permission(name,request): # 权限相关的
...
def get_menu(request): # 菜单相关
    """
    动态生成二级菜单
    :param request:
    :return:
    """
    menu_dict = request.session.get(settings.RBAC_MENU_SESSION_KEY) # 去session里面获取菜单信息
    """
    {
        1: {
            'title': '用户管理',
    ...
    for k,v in menu_dict.items():
        for child in v['children']:
            ...
            if request.default_selected_menu_name == name:
                child['class'] = 'active'  # 菜单默认被选中，并显示
                v['class'] = ''
