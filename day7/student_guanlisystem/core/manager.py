...
class Manager:
    ...
    def __init__(self,name):
        ...
        self.course_pickle=mypickle.MyPickle(settings.course_info)
    ...
    def create_courses(self):
        ...
        self.show_school()
        school_num = int(input('请输入课程所在的学校序号：'))
        course_name= input('请输入课程的名字：')
        school_obj = self.course_pickle.get_item(school_num)  # 根据序号活动了一个学校名字
        couse_obj = Course(school_num,course_name)
        couse_obj.school = school_obj # 将课程对应的的校区和课程对象绑定在一起
        self.course_pickle.dump(couse_obj)
        print('创建课程成功')
    ...
    def create_student(self):
        ...
        #print(cls_obj) course:pythonname:python自动化21,
